...
| **Brier Score** | **0.2477** | Calibrated quadratic probability accuracy reflecting cost-sensitive loss penalization. |
...
### B. Probability of Default (PD) Sigmoidal Transformation
The XGBoost objective function minimizes binary cross-entropy, outputting raw margin scores (log-odds, $z$). For institutional underwriting and credit tiering, raw margins are mapped to a calibrated probability space $P(Y=1) \in [0, 1]$ via the logistic sigmoid link function:

$$P(\text{Default} \mid \mathbf{x}) = \frac{1}{1 + e^{-z}} = \frac{1}{1 + e^{-\left( \sum_{k=1}^K f_k(\mathbf{x}) \right)}}$$

Where $f_k(\mathbf{x})$ denotes the leaf weight assigned by the $k$-th boosting tree.
...
To satisfy regulatory transparency requirements regarding black-box models, local and global feature attribution was conducted using Shapley Additive Explanations (**TreeSHAP**) across a representative sample of 10,000 OOT observations.
...
### C. MLOps Runtime Serialization Defense
Due to an upstream bug in XGBoost 2.x serializing leaf float values as bracketed strings (e.g., `"[5E-1]"`), standard TreeExplainer parsers fail at inference. The pipeline implements a runtime namespace injection patch (`shap.explainers._tree.float = safe_float`) directly inside the engine wrapper to enforce safe deserialization without mutating core library dependencies.
...
The accompanying Streamlit UI serves as a deterministic pricing head-up display (HUD). Production deployment into an active trading desk requires integration with real-time FIX API liquidity streams, dynamic Loss Given Default (LGD) matrices, and intraday National Best Bid and Offer (NBBO) market-maker hedging verification.